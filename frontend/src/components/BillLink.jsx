...
/**
 * Opening a document, from anywhere a customer's documents are listed.
 *
 * Clicking a row ASKS what to do with it rather than deciding. Straight to a
 ...
 *
 * Bills AND receipts, since a payment has a document too. Only the bill had
 * one here, so a customer who needed proof that they had paid - for a
 * landlord, an employer, their own file - had to ring the office for it while
 * the bill they were proving they had settled was a tap away.
 *
 * The PDF routes are token-protected, so a plain <a href> arrives signed out
 * and 401s. Everything here fetches with the auth header and works from the
...
/**
 * Which document a row stands for.
 *
 * A payment row carries a payment_date and a bill does not - and a payment
 * also carries the invoice_no of the bill it settled, so the invoice number
 * cannot be used to tell them apart.
 */
export function docFor(row) {
  if (!row) return null;
  if (row.payment_date) {
    const reference = row.receipt_no || `R${row.id}`;
    return {
      kind: "receipt",
      title: `Receipt ${reference}`,
      url: `/portal/payments/${row.id}/receipt`,
      filename: `receipt-${reference}.pdf`,
    };
  }
  return {
    kind: "bill",
    title: `Bill ${row.invoice_no || ""}`.trim(),
    url: `/portal/invoices/${row.id}/pdf`,
    filename: `${row.invoice_no || `invoice-${row.id}`}.pdf`,
  };
}
...
export function useBillActions() {
  ...
  const run = useCallback(async (mode) => {
    ...
    const doc = docFor(pending);
    if (!doc) return;
    setWorking(mode);
    try {
      const url = await fetchDoc(doc.url);
      if (mode === "print") printBlob(url);
      else downloadBlob(url, doc.filename);
      setTimeout(() => URL.revokeObjectURL(url), 60_000);
      dismiss();
    } catch {
      setWorking("");
      toast.error(`That ${doc.kind} could not be opened. `
